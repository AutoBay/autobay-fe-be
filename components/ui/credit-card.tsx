...
const CardIcons = {
  visa: (
    <svg className="h-6 w-10" viewBox="0 0 40 24">
      ...
      <text fill="white" fontFamily="Arial, sans-serif" fontSize="8" fontWeight="bold" textAnchor="middle" x="20" y="15">
        VISA
      </text>
    </svg>
  ),
  ...
  amex: (
    <svg className="h-6 w-10" viewBox="0 0 40 24">
      ...
      <text fill="white" fontFamily="Arial, sans-serif" fontSize="6" fontWeight="bold" textAnchor="middle" x="20" y="15">
        AMEX
      </text>
    </svg>
  ),
  discover: (
    <svg className="h-6 w-10" viewBox="0 0 40 24">
      ...
      <text fill="white" fontFamily="Arial, sans-serif" fontSize="5" fontWeight="bold" textAnchor="middle" x="20" y="15">
        DISCOVER
      </text>
    </svg>
  ),
...
function CreditCard({ value, onChange, onValidationChange, className, ref, cvvLabel = "CVC", cardStyle = "base", showVendor = true }: CreditCardProps) {
    ...
    <div className={cn("w-full max-w-sm py-2", className)} ref={containerRef}>
      ...
      <div className="relative mb-6 h-56 [perspective:1000px]">
        ...
        >
          {/* Front of Card */}
          <Card
            className={cn("absolute inset-0 flex h-full w-full flex-col justify-between p-6 shadow-xl [backface-visibility:hidden]", cardStyles[cardStyle])}
          >
            <div className="flex items-start justify-between">
              <div className={cn("h-8 w-12 rounded shadow-md", getChipColor())} />
              {/* Vendor logo moved to top right for now, will be repositioned */}
            </div>

            <div className="space-y-4">
              <div className="font-bold font-mono text-xl tracking-wider">{currentValue.cardNumber || "•••• •••• •••• ••••"}</div>

              {/* Bottom row: cardholder - expires - vendor logo */}
              <div className="flex items-end justify-between">
                <div className="flex-1">
                  <div className="font-medium text-xs uppercase opacity-70">Card Holder</div>
                  <div className="font-bold text-sm">{currentValue.cardholderName || "YOUR NAME"}</div>
                </div>
                <div className="flex-1 text-center">
                  <div className="font-medium text-xs uppercase opacity-70">Expires</div>
                  <div className="font-bold text-sm">
                    {currentValue.expiryMonth && currentValue.expiryYear ? `${currentValue.expiryMonth}/${currentValue.expiryYear.slice(-2)}` : "MM/YY"}
                  </div>
                </div>
          ...
          >
            ...
            <div className="flex items-center justify-end space-x-4">
              <div className="text-right">
                <div className="font-medium text-xs uppercase opacity-70">{cvvLabel}</div>
                <div className="rounded bg-white px-3 py-1 text-center font-bold font-mono text-black">{currentValue.cvv || "•••"}</div>
              </div>
              <Lock className="h-6 w-6 opacity-60" />
            </div>

            <div className="text-center font-medium text-xs opacity-60">This card is protected by advanced security features</div>
          </Card>
        </motion.div>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <label className="mb-2 block font-medium text-sm">Cardholder Name</label>
          <Input
            className={cn(
          ...
          />
          {errors.cardholderName && <p className="mt-1 text-destructive text-xs">{errors.cardholderName}</p>}
        </div>

        <div>
          <label className="mb-2 block font-medium text-sm">Card Number</label>
          <Input
            className={cn(
          ...
          />
          {errors.cardNumber && <p className="mt-1 text-destructive text-xs">{errors.cardNumber}</p>}
        </div>

        <div className="grid grid-cols-3 gap-4">
          <div>
            <label className="mb-2 block font-medium text-sm">Month</label>
            <Select onValueChange={(value) => handleInputChange("expiryMonth", value)} value={currentValue.expiryMonth}>
              <SelectTrigger
                className={cn(
                  "transition-all duration-200",
            ...
            {errors.expiryMonth && <p className="mt-1 text-destructive text-xs">{errors.expiryMonth}</p>}
          </div>

          <div>
            <label className="mb-2 block font-medium text-sm">Year</label>
            <Select onValueChange={(value) => handleInputChange("expiryYear", value)} value={currentValue.expiryYear}>
              <SelectTrigger
                className={cn("transition-all duration-200", focusedField === "expiryYear" && "ring-2 ring-primary", errors.expiryYear && "border-destructive")}
              >
            ...
            {errors.expiryYear && <p className="mt-1 text-destructive text-xs">{errors.expiryYear}</p>}
          </div>

          <div>
            <label className="mb-2 block font-medium text-sm">{cvvLabel}</label>
            <Input
              className={cn(
            ...
            />
            {errors.cvv && <p className="mt-1 text-destructive text-xs">{errors.cvv}</p>}
          </div>
        </div>
      </div>
    </div>
